# -*- coding: utf-8 -*-
"""
Created on Sat Oct 26 02:26:23 2024
...
class PolypPVT(nn.Module):
    ...
    def forward(self, x):
        ...
        # if x4.size(2)*2!=x3.size(2):
        #     size=x4.size(2)*2
        #     x3 = F.upsample(x3, size=(size,size), mode='bilinear', align_corners=True)
        #     x2 = F.upsample(x2, size=(size*2,size*2), mode='bilinear', align_corners=True)
        #     x1 = F.upsample(x1, size=(size*4,size*4), mode='bilinear', align_corners=True)
